        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 mb-8 pt-4">
          <div>
            <div className="flex items-center gap-2 md:gap-3 mb-2">
              ...
              <h1 className="font-heading italic text-3xl md:text-5xl tracking-tight text-emerald-950">
                Krishi <span className="text-emerald-700">Chaupal</span>
              </h1>
            </div>
            <p className="text-emerald-800/80 text-sm md:text-base font-medium flex items-center gap-2">
        ...
        <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-hide -mx-4 px-4 md:mx-0 md:px-0">
          {[
            ...
            <button
              ...
              className={`flex items-center gap-2 px-5 py-2.5 md:px-6 md:py-3 rounded-full text-xs md:text-sm font-bold whitespace-nowrap transition-all ${
                activeTab === tab.id 
        ...
        <form onSubmit={handlePost} className="bg-white/80 liquid-glass rounded-2xl md:rounded-3xl p-4 md:p-6 border border-emerald-900/10 shadow-sm relative overflow-hidden">
          ...
          <div className="flex flex-col sm:flex-row gap-3 md:gap-4 mb-4">
            <input 
              ...
              className="flex-1 bg-pink-50/50 rounded-xl px-4 py-3 text-sm md:text-base outline-none focus:bg-white border border-emerald-100 transition-colors"
            />
            <select 
              value={district} onChange={e => setDistrict(e.target.value)}
              className="flex-1 bg-pink-50/50 rounded-xl px-4 py-3 text-sm md:text-base outline-none focus:bg-white border border-emerald-100 text-emerald-900 transition-colors"
            >
          ...
          <textarea 
            ...
            className="w-full bg-pink-50/50 rounded-xl px-4 py-3 text-sm md:text-base outline-none focus:bg-white border border-emerald-100 transition-colors resize-none min-h-[100px] mb-4"
          />
          <div className="flex justify-between items-center">
            ...
            <button 
              disabled={isPosting} 
              type="submit" 
              className="bg-emerald-600 text-white px-5 py-2.5 md:px-6 md:py-2.5 rounded-full text-sm font-bold flex items-center gap-2 hover:bg-emerald-700 disabled:opacity-50 transition-all active:scale-95 shadow-md shadow-emerald-500/20"
            >
              {isPosting ? <Loader2 className="w-4 h-4 animate-spin" /> : <Send className="w-3.5 h-3.5 md:w-4 md:h-4" />}
    ...
    <div className={`bg-white p-5 rounded-2xl border ${post.pinned ? 'border-emerald-400 bg-emerald-50/30' : 'border-emerald-100'} shadow-sm relative group`}>
      ...
      <div className="flex justify-between items-start mb-3">
        <div className="flex gap-2 md:gap-3 items-center">
          <div className="w-9 h-9 md:w-10 md:h-10 rounded-full bg-gradient-to-tr from-emerald-400 to-emerald-600 flex items-center justify-center text-white font-bold text-xs md:text-sm shadow-inner cursor-pointer" onClick={startDM}>
            {post.username.charAt(0).toUpperCase()}
          </div>
          <div>
            <h4 className="font-bold text-emerald-950 text-sm md:text-base flex items-center gap-1 cursor-pointer hover:underline" onClick={startDM}>
              {post.username} 
              {post.isVerified && <Leaf className="w-3 h-3 text-emerald-600" />}
              {!post.isVerified && <Leaf className="w-3 h-3 text-emerald-900/20" />}
            </h4>
            <p className="text-[10px] md:text-xs text-emerald-800/60 font-semibold">
      ...
      <div className="flex items-center gap-3 md:gap-4 text-emerald-800/60 border-t border-emerald-50 pt-3">
        <button onClick={handleLike} className={`flex items-center gap-1.5 text-[10px] md:text-xs font-bold hover:text-emerald-600 transition-colors ${liked ? 'text-emerald-600' : ''}`}>
          <ThumbsUp className={`w-3.5 h-3.5 md:w-4 md:h-4 ${liked ? 'fill-current' : ''}`} /> {localLikes} {t.likes}
        </button>
        <button onClick={startDM} className="flex items-center gap-1.5 text-[10px] md:text-xs font-bold hover:text-emerald-600 transition-colors">
          <MessageCircle className="w-3.5 h-3.5 md:w-4 md:h-4" /> {t.reply}
        </button>
        
        {post.language !== currentLang && translatedMsg === post.message && (
          <button onClick={handleTranslate} className="flex items-center gap-1.5 text-[10px] md:text-xs font-bold text-pink-600 hover:text-pink-700 ml-auto transition-colors">
            {isTranslating ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Globe2 className="w-3.5 h-3.5 md:w-4 md:h-4" />}
            {t.translate}
          </button>
        )}
        
        <button onClick={handleReport} className="flex items-center gap-1 text-xs font-bold hover:text-rose-600 transition-colors ml-auto group/rep">
          <AlertTriangle className="w-3.5 h-3.5 md:w-4 md:h-4 group-hover/rep:fill-rose-100" />
...
function DistrictChats({ t, lang }) {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="flex flex-col md:flex-row gap-0 md:gap-6 bg-white/40 liquid-glass rounded-2xl md:rounded-[2rem] border border-emerald-900/10 overflow-hidden min-h-[500px] h-[80vh] md:h-[70vh]">
      
      {/* Sidebar List */}
      <div className="md:w-64 bg-white/50 border-b md:border-b-0 md:border-r border-emerald-900/10 flex flex-col">
        <div className="p-3 md:p-4 bg-emerald-50/50 border-b border-emerald-900/5 sticky top-0 backdrop-blur-md">
          <h3 className="font-heading italic text-lg md:text-xl text-emerald-950 px-2">{t.districts}</h3>
        </div>
        <div className="p-2 flex md:flex-col gap-1 overflow-x-auto md:overflow-y-auto scrollbar-hide">
          {DISTRICTS.map(d => (
            <button 
              key={d} 
              onClick={() => setActiveDistrict(d)}
              className={`whitespace-nowrap md:whitespace-normal text-left px-4 py-2 md:py-3 rounded-xl font-bold text-xs md:text-sm transition-all ${activeDistrict === d ? 'bg-emerald-100 text-emerald-900 shadow-sm' : 'text-emerald-800 hover:bg-white/60'}`}
            >
              {d}
            </button>
          ))}
      ...
      {/* Chat Area */}
      <div className="flex-1 flex flex-col h-full bg-white/40 min-h-0">
        <div className="p-3 md:p-4 border-b border-emerald-900/10 bg-white/60 flex items-center justify-between">
          <h3 className="font-heading italic text-xl md:text-2xl text-emerald-950 flex items-center gap-2">
            <MapPin className="w-4 h-4 md:w-5 md:h-5 text-emerald-600" /> {activeDistrict}
          </h3>
        </div>

        <div className="flex-1 overflow-y-auto p-4 md:p-6 space-y-4" ref={scrollRef}>
          {messages.map(m => (
            <div key={m.id} className="flex flex-col items-start min-w-[60%] max-w-[90%] md:max-w-[80%]">
               ...
               <div className="bg-white px-4 py-2 md:px-5 md:py-3 rounded-2xl rounded-tl-sm border border-emerald-100 shadow-sm text-sm text-emerald-950">
                 {m.message}
               </div>
            </div>
          ))}
...
function DirectMessaging({ t, lang, dmSessionId, setDmSessionId }) {
  ...
  if (!activeSessionId) {
    return (
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="flex items-center justify-center py-8 md:py-20">
        <div className="bg-white/80 liquid-glass p-6 md:p-8 rounded-2xl md:rounded-[2rem] border border-emerald-900/10 shadow-xl max-w-md w-full text-center">
          <div className="w-12 h-12 md:w-16 md:h-16 bg-emerald-100 text-emerald-600 rounded-full flex items-center justify-center mx-auto mb-4 md:mb-6">
            <MessageCircle className="w-6 h-6 md:w-8 md:h-8" />
          </div>
          <h2 className="font-heading italic text-2xl md:text-3xl text-emerald-950 mb-3 md:mb-4 flex items-center justify-center gap-2">Private Connection</h2>
          <p className="text-emerald-800/80 mb-6 md:mb-8 font-semibold text-xs md:text-sm leading-relaxed">Start a private peer-to-peer chat session without an account. Share the session ID with another farmer to connect.</p>
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="bg-white/40 liquid-glass rounded-2xl md:rounded-[2rem] border border-emerald-900/10 overflow-hidden min-h-[500px] h-[80vh] md:h-[70vh] flex flex-col max-w-4xl mx-auto shadow-sm">
        
      {/* Header */}
      <div className="p-3 md:p-4 border-b border-emerald-900/10 bg-white/60 flex items-center justify-between">
        <div className="flex items-center gap-2 md:gap-3">
            <button onClick={() => setDmSessionId(null)} className="w-8 h-8 md:w-10 md:h-10 rounded-full flex items-center justify-center bg-emerald-50 text-emerald-700 hover:bg-emerald-100">
                <ChevronLeft className="w-4 h-4 md:w-5 md:h-5" />
            </button>
            <div>
              <h3 className="font-heading italic text-lg md:text-xl text-emerald-950 flex items-center gap-2">
                Private Chat
              </h3>
              <p className="text-[8px] md:text-[10px] text-emerald-800/60 font-bold uppercase tracking-widest">ID: {activeSessionId}</p>
            </div>
        </div>
        <button onClick={() => navigator.clipboard.writeText(shareLink)} className="px-3 py-1.5 md:px-4 md:py-2 bg-pink-50 text-emerald-800 rounded-full text-[10px] md:text-xs font-bold hover:bg-pink-100 border border-emerald-100 transition-colors">
            Copy Link
        </button>
      </div>

      <div className="flex-1 overflow-y-auto p-4 md:p-6 space-y-4 bg-white/20" ref={scrollRef}>
        {messages.map(m => {
          const isMe = m.username === name.trim();
          return (
            <div key={m.id} className={`flex flex-col ${isMe ? 'items-end' : 'items-start'} max-w-[90%] md:max-w-[80%] ${isMe ? 'ml-auto' : 'mr-auto'}`}>
                ...
                <div className={`${isMe ? 'bg-emerald-600 text-white rounded-tr-sm' : 'bg-white text-emerald-950 rounded-tl-sm border border-emerald-100'} px-4 py-2 md:px-5 md:py-3 rounded-2xl shadow-sm text-sm break-words`}>
                  {m.message}
                </div>
            </div>
          )
